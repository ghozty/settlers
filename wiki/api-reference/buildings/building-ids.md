...
Complete reference of building names, IDs, and categories for use with `SendServerAction(50, buildingID, grid, 0, null)`.
...
**Usage**:
```javascript
// Detect depleted deposit
if (building.GetBuildingName_string() === "MineDepletedDepositCorn") {
    // Rebuild Farmfield
    game.gi.SendServerAction(50, 43, building.GetGrid(), 0, null);
}
```
...
```javascript
// Build Farmfield at grid position
...
- Building IDs are used with `SendServerAction(50, buildingID, grid, 0, null)` for construction
- Depleted deposit names are detected via `building.GetBuildingName_string()`
- Some Building IDs may vary between game versions
- Always check build queue capacity before building
- Must be on home zone to build
